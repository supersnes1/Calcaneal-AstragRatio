...
df.pca <- prcomp(df)
plot(df.pca$x[,2], df.pca$x[,3], pch = 16, col = habitat.colors, ylab = "PC3", xlab = "PC2")
```
...
```{r}
##Factor Analysis
d.factanal <- factanal(df, factors = 2, scores = 'regression')
```
...
###Local Fisher Discriminant Analysis (LFDA)
```{r}
model1 <- lfda(df, m$habitat, 2, metric="plain")
autoplot(model1, data = m, frame = TRUE, frame.colour = 'habitat', size=0.1)+
  geom_point(shape = ksym) 
```
...
###Kernal Local Fisher Discriminant Analysis (KLFDA)
```{r}
model2 <- klfda(kmatrixGauss(df), m$habitat, 2, metric="plain")
autoplot(model2, data = m, frame = TRUE, frame.colour = 'habitat', size=0.1)+
  geom_point(shape = ksym) 
```
...
###Semi-supervised Local Fisher Discriminant Analysis (SELF)
```{r}
model3 <- self(df, m$habitat, beta = 0.1, r = 2, metric="plain")
autoplot(model3, data = m, frame = TRUE, frame.colour = 'habitat', size=0.1)+
  geom_point(shape = ksym) 
```
...
m.tree <- m[rownames(m) %in% newNameList,]
...
#arrows(6,0.15,6+(df.phylo.pca$L[,1]),0.15+df.phylo.pca$L[,2], col = "red")+
 # annotate("text", x = (df.phylo.pca$L[,1]), y = (df.phylo.pca$L[,2]),
   #  label = rownames(df.phylo.pca$L))
#labels=rownames(df.phylo.pca$L)
