...
####################################################################################################
#2-Habitat Hypothesis (OH+MH and CH)
##PCA
```{r}
library(ggfortify)
m <- m.sp
dropCol <- c(1:3,12:14)
df <- log(m[,-dropCol])
m$habitat <- gsub("MH","OH",m$habitat)
...
lda.tst_MHOH <- lda(x = as.matrix(df), grouping = m$habitat)
plot(lda.tst_MHOH, col = habitat.colors[m$habitat])
```

###Local Fisher Discriminant Analysis (LFDA)
```{r}
model1_MHOH <- lfda(df, m$habitat, 2, metric="plain")
autoplot(model1_MHOH, data = m, frame = TRUE, frame.colour = 'habitat', size=0.1)+
  geom_point(shape = ksym) 
```

```{r eval=FALSE}
autoplot(model1_MHOH, data = m, x=2, y=3,frame = TRUE, frame.colour = 'habitat', size=0.1)+
  geom_point(shape = ksym) 
```
###Kernal Local Fisher Discriminant Analysis (KLFDA)
```{r}
model2_MHOH <- klfda(kmatrixGauss(df), m$habitat, 2, metric="plain")
autoplot(model2_MHOH, data = m, frame = TRUE, frame.colour = 'habitat', size=0.1)+
  geom_point(shape = ksym) 
```

```{r eval=FALSE}
autoplot(model2_MHOH, data = m,x=2,y=3, frame = TRUE, frame.colour = 'habitat', size=0.1)+
  geom_point(shape = ksym) 
```

###Semi-supervised Local Fisher Discriminant Analysis (SELF)
```{r}
model3_MHOH <- self(df, m$habitat, beta = 0.1, r = 2, metric="plain")
autoplot(model3_MHOH, data = m, frame = TRUE, frame.colour = 'habitat', size=0.1)+
  geom_point(shape = ksym) 
```
