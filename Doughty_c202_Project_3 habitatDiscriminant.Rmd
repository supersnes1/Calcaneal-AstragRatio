...
#df.specimen <- scale(x = df.spec, center = df.pca$center, scale = df.pca$scale) %*% df.pca$rotation 

df.specimen2 <- predict(object = df.pca, newdata = df.spec)  #loadings are the same but PC scores are different https://stackoverflow.com/questions/46828500/predict-new-data-into-pca-space-in-r
